~# 2026-03-02 - Mutualists Diff Cure Times
...
experiment_slug <- "2026-03-12-mutual-cure-times"
...
#### Host Num Task Set One Graph

```{r}
half_data <- time_data %>%
  filter(CURE_UPDATES == 100000, update >= 100000)
quarter_data <- time_data %>%
  filter(CURE_UPDATES == 50000, update >= 50000)
three_quarter_data <- time_data %>%
  filter(CURE_UPDATES == 150000, update >= 150000)
no_sym_data <- time_data %>% 
  filter(sym_condition == "no-syms", seed1 >= 160200)
mut_no_cure_data <- time_data %>% 
  filter(CURE_UPDATES == 0, sym_condition == "mutualist")

sliced_time_data <- bind_rows(half_data, quarter_data, three_quarter_data, no_sym_data, mut_no_cure_data)

host_parent_num_task_sets_plt <- sliced_time_data %>%
  filter(update %% 500 == 0) %>%
  ggplot(
    aes(
      x = update,
...
ggsave(
  file = paste(plot_dir, "single_host_num_task_plt.pdf", sep = "/"),
  plot = host_parent_num_task_sets_plt,
  width = 20,
  height = 15
)
...
### Host Entropy Task Set Single Graph
```{r}
host_parent_entropy_task_sets_plt <- sliced_time_data %>%
  filter(update %% 500 == 0) %>%
  ggplot(
    aes(
      x = update,
...
ggsave(
  file = paste(plot_dir, "single_host_parent_entropy_plt.pdf", sep = "/"),
  plot = host_parent_entropy_task_sets_plt,
  width = 20,
  height = 15
)
